...
namespace 新.Buffs
{
	public class 无尽之力 : ModBuff//继承modbuff类
   {
		...
		public override void Update(Player player, ref int buffIndex)//这个函数是当玩家获得BUFF时每帧执行一次，玩家大多数属性每帧重置
		{
			...
			player.GetDamage(DamageClass.Generic) += 0.2f;//攻击力倍率可以加算也可以乘算，但是乘算容易数值膨胀
			player.GetCritChance(DamageClass.Generic) += 20f;//暴击率同理			
			
			player.GetModPlayer<装备效果>().无尽之刃 = true;
		}
	} 
}	