...
namespace 新.Projectiles
{
	public class 水三 : ModProjectile
	{
		...
		public override void SetDefaults()
		{
			...
			Projectile.width =(int)(752*10/3f);//老子总结出碰撞箱规则：16为1格，碰撞箱为0时，鼠标在贴图左上角，横向加16，鼠标向贴图右移一格，
            //纵向加16，鼠标向下移动一格，若碰撞箱纵横等于其贴图分辨率，则鼠标在贴图中心，若scale增加n倍，碰撞箱要改为原来的1/n
			//这样鼠标才会在贴图中心，嗨嗨嗨老子真nb
            Projectile.height = (int)(1073 * 10 / 3f);
        ...
        public override void OnSpawn(IEntitySource source)
        {
            ...
            for (int i =0 ; i < 4; i++)
            {				
				double dd=Math.PI / 2*i;
				float heng = (float)Math.Cos(dd);
				float zong = (float)Math.Sin(dd);
                Vector2 vvel = new Vector2(heng,zong);
                Projectile.NewProjectile(source, Main.MouseWorld-vvel*75f , Vector2.Zero, ProjectileID.DD2ExplosiveTrapT2Explosion, Projectile.damage, Projectile.knockBack, player.whoAmI);
            }
        }
